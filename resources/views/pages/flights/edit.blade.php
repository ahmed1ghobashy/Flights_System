...
@section('body')
    <div class="body-container">
        <div class="credentials-header">
            <span class="header-span">Edit Flight</span>
        </div>
        <div class="credentials-body">
            <form action="{{ url('/flights/' . $flight->id) }}" method="POST">
                ...
                <select id="departure_country_id" name="departure_country_id" class="form-select form-select-lg"
                    aria-label="Large select example" style="border-radius: 7px !important; font-size: 16px;">
                    <option selected disabled>Select Country</option>
                    @foreach ($countries as $country)
                        <option value="{{ $country->id }}" @if ($country->id == $flight->departureCity->country->id) selected @endif>
                            {{ $country->name }}</option>
                    @endforeach
                </select>
                ...
                <select id="departure_city_id" name="departure_city_id" class="form-select form-select-lg"
                    aria-label="Large select example" style="border-radius: 7px !important; font-size: 16px;">
                    <option disabled>Select City</option>
                    @foreach ($flight->departureCity->country->cities as $city)
                        <option value="{{ $city->id }}" @if ($city->id == $flight->departure_city_id) selected @endif>
                            {{ $city->name }}</option>
                    @endforeach
                </select>
                ...
                <select id="arrival_country_id" name="arrival_country_id" class="form-select form-select-lg"
                    aria-label="Large select example" style="border-radius: 7px !important; font-size: 16px;">
                    <option selected disabled>Select Country</option>
                    @foreach ($countries as $country)
                        <option value="{{ $country->id }}" @if ($country->id == $flight->arrivalCity->country->id) selected @endif>
                            {{ $country->name }}</option>
                    @endforeach
                </select>
                ...
                <select id="arrival_city_id" name="arrival_city_id" class="form-select form-select-lg"
                    aria-label="Large select example" style="border-radius: 7px !important; font-size: 16px;">
                    <option selected disabled>Select City</option>
                    @foreach ($flight->arrivalCity->country->cities as $city)
                        <option value="{{ $city->id }}" @if ($city->id == $flight->arrival_city_id) selected @endif>
                            {{ $city->name }}</option>
                    @endforeach
                </select>
                ...
                <input value="{{ $flight->departure_time }}" type="datetime-local" class="form-control" id="datetime" name="departure_time" autocomplete="off">
                @error('departure_time')
                    <div class="text-danger">{{ $message }}</div>
                @enderror

                <label for="arrival_time" class="label mt-3">Arrival Time</label>
                <input value="{{ $flight->arrival_time }}" type="datetime-local" class="form-control" id="datetime" name="arrival_time" autocomplete="off">
                @error('arrival_time')
                    <div class="text-danger">{{ $message }}</div>
                @enderror

                <label for="price" class="label mt-3">Price</label>
                <input value="{{ $flight->price }}" id="price" name="price" type="number" class="form-control" step="0.01">
                @error('price')
                    <div class="text-danger">{{ $message }}</div>
                @enderror

                <label for="available_seats" class="label mt-3">Available seats</label>
                <input value="{{ $flight->available_seats }}" id="available_seats" name="available_seats" type="number" class="form-control">
                ...
                <button class="btn btn-primary submit-btn d-block mt-4" type="submit">Update Flight</button>
            </form>
        </div>
